...
import {
  getContrastClassesForAlerts,
  styles,
} from './dashboard-cognitive-alerts-styles';
...
export function CognitiveAlertBanner({
  ...
  const titleClasses = useMemo(
    () => cn(styles.bannerTitle, fontSizeClasses.base),
    [fontSizeClasses.base]
  );

  const messageClasses = useMemo(
    () => cn(styles.bannerMessage, fontSizeClasses.sm),
    [fontSizeClasses.sm]
  );

  const dismissButtonClasses = useMemo(
    () => cn(styles.dismissButton, animationClasses),
    [animationClasses]
  );

  const iconClasses = useMemo(
    () => cn(styles.icon.color, settings.animations ? styles.icon.animate : ''),
    [settings]
  );
  ...
  return (
    ...
    >
      ...
      <div className={styles.bannerContent}>
        <h3 className={titleClasses}>{getText(titleKey)}</h3>
        <p className={messageClasses}>{getText(messageKey)}</p>
      </div>
