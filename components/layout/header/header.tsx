...
export function Header({ title = 'MindEase' }: HeaderProps) {
  ...
  const headerClasses = useMemo(
    () => cn(styles.header, spacingClasses.padding, fontSizeClasses.base),
  ...
  return (
    <header className={headerClasses} data-testid="header-container">
      <div
        className={cn(styles.branding, spacingClasses.gap)}
      ...
      >
        ...
        <h1 className={titleClasses} data-testid="header-title">
          {title}
        </h1>
      </div>
      <Button
...
const styles = {
  header:
    'h-16 bg-surface-primary border-b border-border-subtle flex items-center justify-between px-6',
