...
export const styles = {
  ...
  contrast: {
    ...
    highActive:
      'bg-action-primary/30 text-action-primary font-semibold border-l-4 border-action-primary shadow-sm',
    highInactive:
      'text-text-primary border-l-4 border-transparent hover:bg-action-primary/15 hover:text-action-primary hover:border-action-primary/50 font-medium',
  } as const,
} as const;
...
export function getContrastClasses(
  ...
  if (contrast === 'high') {
    return isActive ? styles.contrast.highActive : styles.contrast.highInactive;
  }

  return isActive
    ? styles.contrast.normalActive
    : styles.contrast.normalInactive;
}
