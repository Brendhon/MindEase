...
export function useTextDetail() {
  ...
  const getTextWithReplace = useCallback((key: AccessibilityTextKey, replace: Record<string, string>) => {
    const text = getAccessibilityText(key, settings.textDetail);
    return Object.entries(replace).reduce((acc, [key, value]) => acc.replace(`{${key}}`, value), text);
  }, [settings.textDetail]);

  // Memoize getText to minimize re-renders
  const getText = useCallback((key: AccessibilityTextKey) => getAccessibilityText(key, settings.textDetail), [settings.textDetail]);

  // Memoize render to minimize re-renders
  const render = useCallback((detailed: ReactNode, summary: ReactNode) => {
